...
dig2Int :: Char -> Int
dig2Int dChar = indexInto dChar list 
...
int2Base n b = reverse $ tenToBase n b

tenToBase num base
    | num == 0  = ""
    | otherwise = num2char(num `mod` base) : tenToBase (num `div` base) base
...
valNumString (xs) b1 
    | xs == []       = 0
    | otherwise      = (valNumString (tail xs) b1) + (dig2Int(head(xs)) * b1^(length(tail(xs))))
